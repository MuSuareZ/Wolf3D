...
/*   Updated: 2020/03/16 16:39:40 by msuarez-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			init_img(t_env *env)
{
	...
	img = &env->img;
	img->image = mlx_new_image(env->mlx, screenWidth, screenHeight);
...
void			img_pixel_put(t_env *env, double x, double y, int color)
{
	...
	if (x >= 0 && x < screenWidth && y >= 0 && y < screenHeight)
		*(int *)(img->ptr + (int)(idx(y, x, screenWidth) * img->bpp)) = color;
}
...
void			clear_img(t_env *env)
{
	...
	img = &(env->img);
	ft_bzero(img->ptr, (screenWidth * screenHeight) * img->bpp);
}
